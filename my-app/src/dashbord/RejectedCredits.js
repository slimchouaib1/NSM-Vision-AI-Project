...
const RejectedCredits = () => {
  const [rejectedCredits, setRejectedCredits] = useState([]);

useEffect(() => {
  axios.get("http://localhost:8000/api/demandes/")
    .then((res) => {
      setRejectedCredits(res.data.filter(req => req.loan_status === 0));
    })
    .catch((err) => console.error("Failed to fetch rejected credits", err));
}, []);
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 p-6 ml-64 overflow-y-auto h-screen">
        <header className="flex justify-between items-center mb-6 bg-white shadow-md p-4 fixed w-full z-10 top-0 left-64">
          <h1 className="text-xl font-extrabold text-gray-500 tracking-wide uppercase">Rejected Credits</h1>
        </header>

{/* Rejected Credits Table */}
<section className="bg-white p-6 rounded-lg shadow-md mt-8 max-h-[calc(100vh-4rem)] overflow-auto">
  <h3 className="text-lg text-red-600 font-bold mb-4">Rejected Credit Applications</h3>
    ...
    <table className="w-full table-auto border-collapse">
      <thead>
        <tr className="bg-gray-100 text-gray-800">
          ...
          <th className="text-left p-4">Rejection Date</th>
      ...
      <tbody>
        {rejectedCredits.length === 0 ? (
          <tr>
            <td className="p-4 text-center text-gray-500" colSpan="5">No rejected credits</td>
          </tr>
        ) : (
          rejectedCredits.map((credit) => (
            <tr key={credit._id} className="border-t border-gray-200 hover:bg-gray-50 transition">
              ...
              <td className="p-4">{new Date(credit.updatedAt || credit.createdAt).toLocaleDateString()}</td>
              <td className="p-4">
...
export default RejectedCredits;